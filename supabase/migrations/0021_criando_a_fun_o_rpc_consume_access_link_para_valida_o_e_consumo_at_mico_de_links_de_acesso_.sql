-- Migration 0021: Criando a função RPC consume_access_link

CREATE OR REPLACE FUNCTION public.consume_access_link(p_token_hash text, p_visitor_name text DEFAULT NULL, p_visitor_email text DEFAULT NULL, p_user_id uuid DEFAULT NULL, p_user_agent text DEFAULT NULL, p_ip text DEFAULT NULL)
...
BEGIN
    ...
    IF v_link_active = FALSE THEN
        RETURN QUERY SELECT FALSE, 'INACTIVE_LINK', 'Link inativo.', NULL, NULL, NULL, NULL;
        RETURN;
    END IF;

    IF v_link_expires_at IS NOT NULL AND v_now >= v_link_expires_at THEN
        RETURN QUERY SELECT FALSE, 'EXPIRED_LINK', 'Link expirado.', NULL, NULL, NULL, NULL;
        RETURN;
    END IF;

    IF v_link_max_uses IS NOT NULL AND v_link_uses >= v_link_max_uses THEN
        RETURN QUERY SELECT FALSE, 'MAX_USES', 'Limite de usos atingido.', NULL, NULL, NULL, NULL;
        RETURN;
    END IF;
    ...
    -- 4. Registro da Visita (Não-atômico, mas importante)
