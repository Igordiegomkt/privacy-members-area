...
export const MediaItem: React.FC<MediaItemProps> = ({ media, onClick, onLoad }) => {
  ...
  return (
    <div
      ref={containerRef}
      className="relative aspect-square cursor-pointer group overflow-hidden bg-dark-lighter"
      onClick={onClick}
    >
      {hasError ? (
        <div className="w-full h-full flex items-center justify-center bg-dark-lighter text-gray-500 text-xs p-2">
          Erro ao carregar
        </div>
      ) : (
        <>
