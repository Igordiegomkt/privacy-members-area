...
export const MyPurchases: React.FC = () => {
  const { purchases, isLoading: loading } = usePurchases();
  const navigate = useNavigate();

  // 🔐 Garante que só lidamos com compras que realmente têm products
  const purchasesWithProduct = purchases.filter((p): p is NonNullable<typeof p> & { products: NonNullable<typeof p['products']> } => p.products !== null);
