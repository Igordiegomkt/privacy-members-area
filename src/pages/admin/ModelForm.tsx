    ...
    <div>
      ...
      <form onSubmit={handleSubmit} className="bg-privacy-surface p-8 rounded-lg space-y-6 max-w-2xl">
        ...
        <div>
          ...
          <input type="text" name="name" value={model.name || ''} onChange={handleChange} className="w-full input-style" required />
        </div>
        <div>
          <label htmlFor="username" className="block text-sm font-medium text-privacy-text-secondary mb-2">Username (sem @)</label>
          <input type="text" name="username" value={model.username || ''} onChange={handleChange} className="w-full input-style" required />
        </div>
        <div>
          <label htmlFor="bio" className="block text-sm font-medium text-privacy-text-secondary mb-2">Bio</label>
          <textarea name="bio" value={model.bio || ''} onChange={handleChange} className="w-full input-style" rows={4}></textarea>
        </div>
        <div>
          <label htmlFor="avatar_url" className="block text-sm font-medium text-privacy-text-secondary mb-2">URL do Avatar</label>
          <input type="url" name="avatar_url" value={model.avatar_url || ''} onChange={handleChange} className="w-full input-style" />
        </div>
        <div>
          <label htmlFor="cover_url" className="block text-sm font-medium text-privacy-text-secondary mb-2">URL da Capa (Cover)</label>
          <input type="url" name="cover_url" value={model.cover_url || ''} onChange={handleChange} className="w-full input-style" />
        </div>
