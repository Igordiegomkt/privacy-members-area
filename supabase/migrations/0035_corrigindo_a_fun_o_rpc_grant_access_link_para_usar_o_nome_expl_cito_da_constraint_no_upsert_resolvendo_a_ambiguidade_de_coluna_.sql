CREATE OR REPLACE FUNCTION public.grant_access_link(p_token_hash text, p_visitor_name text DEFAULT NULL::text, p_visitor_email text DEFAULT NULL::text, p_user_id uuid DEFAULT NULL::uuid, p_user_agent text DEFAULT NULL::text, p_ip text DEFAULT NULL::text)
...
BEGIN
  ...
  v_resolved_product_id := v_link.product_id;
  
  -- Se o link é por modelo (scope='model') e não tem product_id definido, busca o base_membership daquela modelo
  IF v_resolved_product_id IS NULL AND v_link.model_id IS NOT NULL AND v_link.scope = 'model' THEN
    SELECT p.id, p.price_cents, p.name
    INTO v_resolved_product_id, v_price_cents, v_product_name
    FROM public.products p
    WHERE p.model_id = v_link.model_id -- FILTRO CRÍTICO: Apenas produtos da modelo do link
      AND p.is_base_membership = TRUE
      AND p.status = 'active'
    ORDER BY p.created_at ASC -- Pega o mais antigo se houver mais de um (fallback)
    LIMIT 1;
  ELSIF v_resolved_product_id IS NOT NULL THEN
    -- Se tem produto (scope='product' ou product_id já definido), busca o preço
    SELECT p.price_cents, p.name
    INTO v_price_cents, v_product_name
    FROM public.products p
    WHERE p.id = v_resolved_product_id
    LIMIT 1;
  END IF;
  ...
  ON CONFLICT ON CONSTRAINT user_purchases_user_id_product_id_key DO UPDATE -- CORREÇÃO APLICADA AQUI
  SET
    status = 'paid',
    paid_at = NOW(),
    payment_provider = 'external_link'
  WHERE public.user_purchases.status != 'paid'; -- Atualiza apenas se não estiver pago
  ...
  RETURN QUERY SELECT
    ...
    v_link.scope::text,
    v_link.model_id,
    v_resolved_product_id, -- Retorna o product_id resolvido
    v_link.expires_at;
END;
$function$;